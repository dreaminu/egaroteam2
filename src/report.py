...
def df_to_html_table(
    ...
    table = view.to_html(index=False, classes="data-table", border=0)
    return f"<div class='table-wrap'>{table}</div>"


COMBO_COLS = ["법정동", "도로명", "건축년도구간", "면적구간", "거래건수", "평균거래금액_만원", "평균평당가_만원", "최근거래일"]
ROAD_COLS = ["법정동", "도로명", "거래건수", "평균거래금액_만원", "평균평당가_만원", "최근거래일", "대표면적구간", "대표연식구간"]
FOLLOW_COLS = ["법정동", "도로명", "건축년도구간", "면적구간", "거래건수", "평균평당가_만원", "동평균평당가_만원", "평당가비교", "최근거래일"]
...
<head>
  ...
  <style>
    * {{ box-sizing: border-box; }}
    ...
    table.data-table {{ width: 100%; border-collapse: collapse; font-size: 13.5px; border-radius: 14px; }}
    table.data-table th {{ background: #1e3a8a; color: white; text-align: left; padding: 9px 10px; white-space: nowrap; }}
    table.data-table td {{ border-bottom: 1px solid #e5e7eb; padding: 8px 10px; white-space: nowrap; }}
    table.data-table th:nth-child(2), table.data-table td:nth-child(2) {{ min-width: 150px; }}
    ...
    @media print {{
      ...
      .card {{ box-shadow: none; border: 1px solid #e2e8f0; border-radius: 10px; page-break-inside: avoid; }}
      .card.long {{ page-break-inside: auto; }}
      .two {{ grid-template-columns: 1fr; }}
      .table-wrap {{ overflow: visible; }}
      table.data-table {{ font-size: 10px; }}
      table.data-table th {{ background: #1e3a8a !important; -webkit-print-color-adjust: exact; print-color-adjust: exact; padding: 4px 6px; }}
      table.data-table td {{ padding: 4px 6px; white-space: normal; }}
      table.data-table th:nth-child(2), table.data-table td:nth-child(2) {{ min-width: 110px; white-space: nowrap; }}
...
<div class="page">
  ...
  <div class="card long"><h2>핵심: 도로명 + 연식 + 면적대 조합 TOP 20</h2>{df_to_html_table(analysis.get('road_year_area_combo'), 20, COMBO_COLS)}</div>
  <div class="card long"><h2>도로명별 거래량 TOP 20</h2>{df_to_html_table(analysis.get('road_volume'), 20, ROAD_COLS)}</div>
  <div class="card long"><h2>추가조사 후보 TOP 20</h2>{df_to_html_table(analysis.get('follow_up_candidates'), 20, FOLLOW_COLS)}</div>
  <div class="card"><h2>거래량 많은 동네 + 평수대 TOP 20</h2>{df_to_html_table(analysis.get('top_dong_area'), 20)}</div>
  <div class="card long"><h2>도로명 + 연식별 분석 TOP 20</h2>{df_to_html_table(analysis.get('road_build_year'), 20, COMBO_COLS)}</div>
  <div class="card long"><h2>도로명 + 면적대별 분석 TOP 20</h2>{df_to_html_table(analysis.get('road_area'), 20, COMBO_COLS)}</div>
  <div class="two">
